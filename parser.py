#RAW데이터를 각 시트에 지정된 컬럼에 넣는 코드 (Put&Call 제외)
import os
import re
import json
...
def extract_bond_strike_price_from_section9(
        ...
        for k, v in corr_after.items():
            ...
            if any(lb in k_norm for lb in label_norms):
                ...
                for lb in price_labels:
                    m = re.search(rf"{re.escape(lb)}\s*[:：]?\s*([0-9][0-9,]*)", merged)
                    if m:
                        num = int(m.group(1).replace(",", ""))
                        if _valid_price(num):
                            return f"{num:,}"

                num = _max_int_in_text(merged)
                if _valid_price(num):
                    return f"{num:,}"
    ...
    if block_rows:
        ...
        for lb in price_labels:
            m = re.search(rf"{re.escape(lb)}\s*[:：]?\s*([0-9][0-9,]*)", block_text)
            if m:
                num = int(m.group(1).replace(",", ""))
                if _valid_price(num):
                    return f"{num:,}"
...
def parse_bond_record(rec: Dict[str, Any]):
    ...
    row["행사(전환)가액(원)"] = extract_bond_strike_price_from_section9(
        tables,
        corr_after,
        row["구분"],
    )
    ...
    if strike_price_num is not None and (strike_price_num < 50 or strike_price_num > 100000000):
